...
setup_cookies() {
    ...
    if [[ -f "cookies.txt" ]]; then
        if [[ -s "cookies.txt" ]]; then
            ...
            echo -e "${RED}cookies.txt found but is empty!${NC}"
            read -p "Add cookies to cookies.txt? (y/n): " add_cookies
    ...
    # Either no cookies.txt or user chose to add cookies
    ...
    read -p "Enable cookies? (y/n): " need_cookies
    ...
    read -p "Choice (1-2): " cookie_method
...
configure_editing() {
    ...
    local options=("No editing" "Trim video" "Resize video" "Convert format" "Flip video")
...
main() {
    ...
    local platform_options=("Facebook" "YouTube" "TikTok" "Other")
    ...
    case $platform in
        tiktok) extra_options="--force-overwrites --write-description --write-thumbnail";;
        youtube) extra_options="--embed-thumbnail --add-metadata --write-description";;
        *) extra_options="";;
    esac
    
    # Show summary
    show_summary "$platform" "$url_identifier" "$quality_label" "$output_dir" "$quality_choice"
