...
setup_cookies() {
    ...
    echo -e "\n${BOLD}Cookie Setup Options:${NC}"
    echo -e "1) Provide cookies.txt path"
    echo -e "2) Skip cookie setup"
    read -p "Select (1-2): " cookie_method
    
    case $cookie_method in
        1)
            read -p "Enter full path to cookies.txt: " cookie_path
            if [[ -f "$cookie_path" && -s "$cookie_path" ]]; then
                if grep -q "^#.*Netscape HTTP Cookie File" "$cookie_path"; then
                    cp "$cookie_path" ./cookies.txt
                    cookies_command="--cookies cookies.txt"
                    echo -e "${GREEN}Cookies configured!${NC}"
                else
                    echo -e "${RED}Provided file does not contain valid cookies! Continuing without cookies.${NC}"
                    cookies_command=""
                fi
            else
...
get_url_identifier() {
    ...
    case $platform in
        youtube)
            ...
            elif [[ $url == *"youtube.com/watch"* || $url == *"youtu.be"* || $url == *"youtube.com/playlist"* ]]; then
                echo "videos"
            else
                echo "videos"
            fi
...
show_summary() {
    ...
    if [[ -n "$playlist_range" ]]; then
        echo -e " - Video Range: ${playlist_range}${NC}"
    fi
...
main() {
    ...
    case $platform_choice in
        1) platform="youtube"; profile_type="video/channel/playlist";;
    ...
    # Playlist range selection for "All videos"
    playlist_command=""
    playlist_range=""
    if [[ "$scope_choice" == "2" && "$platform" == "youtube" ]]; then
        echo -e "${YELLOW}Checking total videos in $profile_type...${NC}"
        total_videos=$(yt-dlp $cookies_command --get-id --flat-playlist "$video_url" | wc -l)
        if [[ $total_videos -eq 0 ]]; then
            echo -e "${RED}No videos found or URL is invalid!${NC}"
            exit 1
        fi
        echo -e "${GREEN}Found $total_videos videos.${NC}"
        read -p "${YELLOW}Enter video range to download (e.g., 5-20 or 1-$total_videos for all): ${NC}" range_input
        until [[ "$range_input" =~ ^[0-9]+-[0-9]+$ ]]; do
            echo -e "${RED}Invalid range format. Use start-end (e.g., 5-20).${NC}"
            read -p "${YELLOW}Enter video range (e.g., 5-20): ${NC}" range_input
        done
        start=$(echo "$range_input" | cut -d'-' -f1)
        end=$(echo "$range_input" | cut -d'-' -f2)
        until [[ "$start" -ge 1 && "$end" -ge "$start" && "$end" -le "$total_videos" ]]; do
            echo -e "${RED}Invalid range. Start must be >= 1, end >= start, and end <= $total_videos.${NC}"
            read -p "${YELLOW}Enter video range (e.g., 5-20): ${NC}" range_input
            start=$(echo "$range_input" | cut -d'-' -f1)
            end=$(echo "$range_input" | cut -d'-' -f2)
        done
        playlist_command="--playlist-start $start --playlist-end $end"
    ...
    # Description file selection (only for video downloads)
    description_command=""
    ...
    # Show summary
    show_summary "$platform" "$url_identifier" "$quality_label" "$output_dir" "$quality_choice" "$description_choice" "$playlist_range"
    ...
    if [[ "$scope_choice" == "1" && -n "$edit_command" ]]; then
        ...
        echo -e "${YELLOW}Downloading videos $playlist_range...${NC}"
        if ! yt-dlp $cookies_command $extra_options $playlist_command -f "$quality" -o "$output_template" --yes-playlist "$video_url"; then
            echo -e "${RED}Download failed!${NC}"
            exit 1
        fi
    fi
