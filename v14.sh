...
setup_cookies() {
    ...
    echo -e "1. Log in to YouTube in Chrome/Firefox."
    ...
    echo -e "3. Export cookies and paste below (Netscape format)."
    echo -e "4. Press Enter twice to skip cookies and download public videos only.\n"
    
    echo -e "${YELLOW}Paste cookies below (end with empty line):${NC}"
    cookies=""
    ...
    if [[ -n "$cookies" ]]; then
        # Save cookies exactly as pasted to cookies.txt
        echo -e "$cookies" > cookies.txt
        echo -e "${GREEN}Cookies saved to cookies.txt${NC}"
