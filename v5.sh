...
# -----------------------------------------------
#   FUNCTION: Display download summary
# -----------------------------------------------
show_summary() {
    local platform=$1
...
main() {
    ...
    if [[ "$scope_choice" == "2" ]]; then
        ...
        # For single video, include original URL identifier in filename
    ...
    show_summary "$platform" "$url_identifier" "$quality_choice" "$output_dir"
