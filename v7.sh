...
main() {
    ...
    if [[ "$scope_choice" == "1" && -n "$edit_command" && $has_ffmpeg -eq 0 ]]; then
        ...
        if ! ffmpeg -i "$output_template" $edit_command "$edit_output_template" 2> ffmpeg_error.log; then
            echo -e "${RED}[✗] Editing failed. Check ffmpeg_error.log for details.${NC}"
        ...
        echo -e "${YELLOW}[+] Downloading playlist/channel...${NC}"
